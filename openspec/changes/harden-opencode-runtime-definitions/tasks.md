...
- [ ] 1.2 RED: add failing cases under `scripts/validate/fixtures/runtime-definitions/` for missing skill frontmatter, directory/name/manifest mismatch, duplicate catalog IDs, unresolved runtime paths, invalid phase mappings, self-submodule declarations, and tracked `Zone.Identifier` files.
...
- [ ] 2.3 Add `core/templates/opencode.consumer.json` with `$schema` and `default_agent: "orchestrator"`, plus safe merge guidance for consumers with existing configuration.
...
- [ ] 3.1 RED: add routing tests for planning versus standalone design, implementation plus documentation, Pulumi verification, refactor from OpenSpec tasks, TDD delegation, unknown specialization fallback, and exactly-one-phase selection.
...
- [ ] 4.3 Configure feature-iteration, Pulumi, and TDD subagents with `model: openai/gpt-5.6-sol`, `steps: 20`, and `permission.task: deny`; also deny nested delegation for Luna leaf subagents.
- [ ] 4.4 Remove Go, Lambda, API Gateway, RFC 7807, `apiManagment.yml`, Pulumi, and repository-path assumptions from shared subagents; obtain those rules from the active pack and specialization skills instead.
- [ ] 4.5 Resolve test ownership by making the TDD helper responsible for test design and test edits, keeping production changes with the orchestrator unless explicitly delegated, and allowing feature iteration to update but not weaken valid tests.
...
## 5. Runner Root and Pack Composition

- [ ] 5.1 RED: add source-layout and consumer-layout tests proving that module definitions and consumer OpenSpec artifacts resolve from different roots.
...
- [ ] 5.3 Validate requested pack names and embed the selected `pack.yaml` constraints, verification commands, TDD commands, and prohibited actions in generated runtime context.
- [ ] 5.4 GREEN: verify source and consumer bundles resolve the expected absolute roots and that unknown packs fail with an actionable error.
...
- [ ] 7.2 Add a migration table from removed primary phase agents to orchestrator-plus-phase-skill execution and document Sol/Luna responsibilities and step limits.
...
- [ ] 8.4 Run representative source and consumer bundles for planning, implementation, verification, and archive phases; record module/project paths, selected phase skill, pack content, references mode, artifacts, and size diagnostics.
